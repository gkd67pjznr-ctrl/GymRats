...
## SPEC Status Update After Sync

After documentation sync completes successfully:

1. **Determine if in worktree**:
```bash
   CURRENT_DIR=$(pwd)
   if [[ "$CURRENT_DIR" == *"/worktrees/"* ]]; then
       # We're in a worktree, need to update main repo
...
2. **Update STATUS.md in main repo**:
```bash
   cd "$MAIN_REPO"
...
3. **Report to user**:
```
   ✅ SPEC-XXX-NNN sync complete
   ✅ STATUS.md updated in main repository
   
   Next steps:
   - Review changes in worktree
   - Run: moai-wt done SPEC-XXX-NNN (to merge)
```
