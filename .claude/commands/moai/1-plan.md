...
## SPEC Status Pre-Check

Before creating SPEC, execute:
```bash
# Check if STATUS.md exists and is readable
if [ -f .moai/specs/STATUS.md ]; then
    python .moai/scripts/sync-status.py
    cat .moai/specs/STATUS.md
fi
```

**Display current SPECs to user**:
- List all existing SPECs with status
- Ask user to confirm no overlap with new feature

**If overlap detected**:
```
⚠️  Similar feature found in SPEC-AUTH-001 (status: completed)
   
   Options:
   A) Extend SPEC-AUTH-001 (create SPEC-AUTH-002)
   B) Create independent SPEC
   C) Cancel and revise
...
[... existing 1-plan command content ...]

---

## SPEC Status Post-Update

After SPEC creation succeeds:
...
2. **Confirm to user**:
```
   ✅ SPEC-{DOMAIN}-{NNN} created
   ✅ Worktree created: ~/moai/worktrees/{project}/SPEC-{DOMAIN}-{NNN}
   ✅ STATUS.md updated
